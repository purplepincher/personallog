# personallog — landing page Worker for personallog.ai

> ✅ **What this repo actually is:** a tiny Cloudflare Worker that serves a
> single static HTML page (the landing page for the `personallog.ai` domain).
...
`personallog-agent`, a thin "PLATO domain agent" for personal notes. That package
is published on PyPI and its source lives in a **separate** repo
([`SuperInstance/personallog-agent`](https://github.com/SuperInstance/personallog-agent)).
...
├── wrangler.jsonc      # Cloudflare Workers config: name="personallog", assets dir=./public
...
✅ Verified: `npx wrangler deploy --dry-run` succeeds — it reads
`public/index.html` as the single asset and binds `env.ASSETS`. There is **no
test suite** in this repo and no `package.json`; nothing to install beyond
Wrangler itself.
...
- ✅ **Real today:** builds and serves a static landing page via a Cloudflare
  Worker (dry-run verified). The HTML/CSS is real and self-contained.
- 🔮 **Not in this repo:** there is no PLATO integration, no agent logic, no
  note-writing/querying code, and no build pipeline beyond Wrangler. The
...
About the **advertised package** `personallog-agent` (separate repo/PyPI — I
verified these against PyPI and the upstream GitHub README, not assumed):

- ✅ **Real today:** genuinely published on PyPI as `personallog-agent` `0.1.0`
  (summary: *"personallog domain agent for PLATO fleet"*). HTTP 200 on
  `https://pypi.org/pypi/personallog-agent/json`.
- ⚠️ **Real but thin:** the upstream README (`SuperInstance/personallog-agent`)
  has a title, a one-line description, an install line, and an **empty `Usage`
  section**. So "install it and use it" is real, but there is no documented API
  example yet. The landing page's own banner calls this a **"THIN SEED"** /
  "placeholder with almost no implemented surface yet" — that framing is
  accurate.
